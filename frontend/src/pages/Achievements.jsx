...
export default function Achievements() {
    ...
    return (
        <div className="max-w-6xl mx-auto font-sans animate-fade-in pb-10">
            ...
            {/* Header */}
            <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[var(--border-color)]">
                <div>
                    ...
                    <p className="text-[var(--text-secondary)]">Track your productivity metrics and consistancy.</p>
                </div>

                <div className="flex items-center gap-2 bg-[var(--bg-secondary)] px-4 py-2 rounded-xl border border-[var(--border-color)] shadow-sm">
                    <span className="text-[var(--text-secondary)] text-sm font-medium">Productivity Level</span>
                    <span className="text-[var(--text-primary)] font-bold text-lg">{user.level}</span>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <section>
                        ...
                        <div className="bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-color)] overflow-hidden shadow-sm">
                            {completedTasks.length === 0 ? (
                                <div className="p-8 text-center text-[var(--text-secondary)]">No completed tasks yet.</div>
                            ) : (
                                completedTasks.slice(0, 5).map((task, i) => (
                                    <div key={task._id} className="p-4 border-b border-[var(--border-color)] last:border-0 flex items-center hover:bg-[var(--bg-tertiary)] transition">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-[var(--text-primary)] font-medium truncate">{task.title}</h4>
                                            <p className="text-xs text-[var(--text-secondary)]">
                                                {new Date(task.completedAt).toLocaleDateString()} • {task.duration ? `${task.duration}m focus` : 'Quick task'}
                                            </p>
                                        </div>
                                        <span className="text-xs font-mono text-[var(--text-secondary)] bg-[var(--bg-tertiary)] px-2 py-1 rounded">+{50 + (task.duration || 0)} XP</span>
                                    </div>
                                ))
                            )}
                        </div>
                ...
                <div className="space-y-6">
                    <div className="bg-gradient-to-br from-[#1d9bf0]/10 to-purple-500/5 p-6 rounded-2xl border border-[#1d9bf0]/20 relative overflow-hidden">
                    ...
                    <div className="bg-[var(--bg-secondary)] p-6 rounded-2xl border border-[var(--border-color)] shadow-sm">
                        <h3 className="text-[var(--text-primary)] font-bold mb-4">Level Progress</h3>
                        <div className="flex justify-between text-sm mb-2">
                            <span className="text-[var(--text-secondary)]">Lvl {user.level}</span>
                            <span className="text-[var(--text-secondary)]">Lvl {user.level + 1}</span>
                        </div>
...
function StatCard({ title, value, icon, trend }) {
    return (
        <div className="bg-[var(--bg-secondary)] p-6 rounded-2xl border border-[var(--border-color)] hover:border-[#1d9bf0]/30 transition group shadow-sm hover:shadow-md">
            <div className="flex justify-between items-start mb-4">
                <div className="text-[var(--text-secondary)]">{title}</div>
                <div className="p-2 bg-[var(--bg-tertiary)] rounded-lg group-hover:bg-[#1d9bf0]/10 transition">{icon}</div>
            </div>
            <div className="text-3xl font-bold text-[var(--text-primary)] mb-1">{value}</div>
            <div className="text-xs text-[var(--text-secondary)] font-mono">{trend}</div>
        </div>
    )
}
...
function MilestoneCard({ milestone }) {
    ...
    return (
        <div className={`p-5 rounded-2xl border transition ${isUnlocked ? 'bg-[#1d9bf0]/10 border-[#1d9bf0]/30' : 'bg-[var(--bg-secondary)] border-[var(--border-color)]'}`}>
            <div className="flex items-center gap-3 mb-3">
                <div className={`p-2 rounded-full ${isUnlocked ? 'bg-[#1d9bf0] text-white' : 'bg-[var(--bg-tertiary)] text-[var(--text-secondary)]'}`}>
                    {milestone.icon}
                </div>
                <div>
                    <h4 className={`font-bold ${isUnlocked ? 'text-[#1d9bf0]' : 'text-[var(--text-primary)]'}`}>{milestone.label}</h4>
                    <p className="text-xs text-[var(--text-secondary)]">Target: {milestone.target}{milestone.suffix || ""}</p>
                </div>
            </div>
            <div className="h-1.5 bg-[var(--bg-tertiary)] rounded-full overflow-hidden">
                <div
                    className={`h-full transition-all duration-1000 ${isUnlocked ? 'bg-[#1d9bf0]' : 'bg-[var(--text-secondary)]'}`}
